...
mkdir $diretorio_mnt/$(date +%d-%m-%y)/$porta &>> /home/bkp_dedicado/$porta.log;
cd $diretorio_mnt/$(date +%d-%m-%y)/$porta &>> /home/bkp_dedicado/$porta.log;
echo "" >> /home/bkp_dedicado/$porta.log;
echo "$(date +%H:%M:%S) - Limpando LOG WAL antigo" >> /home/bkp_dedicado/$porta.log;
/usr/lib/postgresql/$versao/bin/pg_archivecleanup "${DIR_LOGS}" "${LAST_LOG}"
echo "" >> /home/bkp_dedicado/$porta.log;
